...
/// A [GoRouter] instance exposed as a Riverpod [Provider].
final appRouterProvider = Provider<GoRouter>((ref) {
  final authListenable = _SupabaseAuthListenable();
  ...
  return GoRouter(
    ...
    redirect: (BuildContext context, GoRouterState state) {
      ...
      final isPublic = loc == AppConstants.routeLogin ||
          loc == AppConstants.routeRegister;
      final isClientApp = loc == AppConstants.routeClientApp;

      // Always allow register and client app
      if (loc == AppConstants.routeRegister) return null;
      if (isClientApp) return null;

      if (!authed && !isPublic) return AppConstants.routeLogin;
    ...
    // ── Routes ───────────────────────────────────────────────────────────────
    routes: [
      // Public routes (no shell / sidebar)
      GoRoute(
      ...
      // Client-facing ordering app (kiosk / customer screen)
      GoRoute(
        path: AppConstants.routeClientApp,
        name: 'client',
        pageBuilder: (context, state) => _fadePage(
          state: state,
          child: const ClientAppScreen(),
        ),
      ),
      ...
      ShellRoute(
        ...
        routes: [
          ...
          // Inventory with nested product form
          GoRoute(
            ...
            routes: [
              GoRoute(
                // Optional :id param – omit for new product, provide for edit.
                path: 'product',
                name: 'product-form',
...
/// A [CustomTransitionPage] with a simple fade transition.
...
/// A [CustomTransitionPage] with a subtle slide-up-and-fade transition.
...
/// The persistent shell that wraps all authenticated screens.
///
/// On wide screens (≥ [AppConstants.sidebarBreakpoint]) the [SidebarNav] is
/// shown as an inline [NavigationRail]; on narrow screens it lives inside a
/// [Drawer].
class AppShell extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final isWide = width >= AppConstants.sidebarBreakpoint;
